...
export async function startRun(spec: RuntimeSpec, code: string, onLine: OnLine): Promise<RunHandle> {
  switch (spec.engine) {
    ...
    case "clangxx": {
      const { runCpp } = await import("./engineC");
      return { done: runCpp(code, onLine, spec.loadNote), cancel: () => {} };
    }
    case "yaegi": {
    ...
    case "shell": {
      const { runLinux } = await import("./engineLinux");
      return { done: runLinux(code, onLine, spec.loadNote), cancel: () => {} };
    }
    case "git": {
